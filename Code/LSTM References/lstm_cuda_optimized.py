...
def construct_qs_matrix(q_ids_sequential, dict_sequence_lengths, candidates=False):
    ...
    qs_padded = padded_q_matrix(qs_matrix_list).cuda()
    h0 = Variable(torch.zeros(1, qs_padded.size()[0], hidden_size), requires_grad=True).cuda()
    c0 = Variable(torch.zeros(1, qs_padded.size()[0], hidden_size), requires_grad=True).cuda()
